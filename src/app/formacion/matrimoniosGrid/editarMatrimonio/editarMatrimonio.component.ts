...
export class EditarMatrimonioComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.obtenerMatrimonio(elementId).subscribe((data) => {
      ...
      this.editarMatrimonioForm.controls["fechaCreacion"].setValue(
        fechaFormateada
      );
      this.editarMatrimonioForm.controls["jornadaDialogo"].setValue(
        data.response.jornadaDialogo
      );
      this.editarMatrimonioForm.controls["retornoEspiritual"].setValue(
        data.response.retornoEspiritual
      );
      this.editarMatrimonioForm.controls["lenguajeAmor"].setValue(
        data.response.lenguajeAmor
      );
      this.editarMatrimonioForm.controls["guiaDeRelacion"].setValue(
        data.response.guiaDeRelacion
      );
      this.editarMatrimonioForm.controls["sacramento"].setValue(
        data.response.sacramento
      );
      this.editarMatrimonioForm.controls["diosEnSacramento"].setValue(
        data.response.diosEnSacramento
      );
      this.editarMatrimonioForm.controls["diosEnVida"].setValue(
        data.response.diosEnVida
      );
      this.editarMatrimonioForm.controls["patronesComportamiento"].setValue(
        data.response.patronesComportamiento
      );
      this.editarMatrimonioForm.controls["dialogoProfundo"].setValue(
        data.response.dialogoProfundo
      );
      this.editarMatrimonioForm.controls["servidoresPostEncuentro"].setValue(
        data.response.servidoresPostEncuentro
      );
      this.editarMatrimonioForm.controls["formacionAcompanantes"].setValue(
        data.response.formacionAcompanantes
      );
      this.editarMatrimonioForm.controls["padreNuestro"].setValue(
        data.response.padreNuestro
      );
      this.editarMatrimonioForm.controls["transmisionNacional"].setValue(
        data.response.transmisionNacional
      );
      ...
      this.obtenerDatosPais(pais.id).subscribe((data: any) => {
        ...
        const selectPais = document.getElementById(
          "select-pais"
  ...
  editarMatrimonio() {
    ...
    const fecha = (document.getElementById("fechaCreacion") as HTMLInputElement)
      .value;
    const fechaObjeto = new Date(fecha);
    const fechaCreacion = fechaObjeto.toISOString();
    const jornadaDialogo = (<HTMLInputElement>(
      document.getElementById("jornadaDialogo")
    )).value;
    const retornoEspiritual = (<HTMLInputElement>(
      document.getElementById("retornoEspiritual")
    )).value;
    const lenguajeAmor = (<HTMLInputElement>(
      document.getElementById("lenguajeAmor")
    )).value;
    const guiaDeRelacion = (<HTMLInputElement>(
      document.getElementById("guiaDeRelacion")
    )).value;
    const sacramento = (<HTMLInputElement>document.getElementById("sacramento"))
      .value;
    const diosEnSacramento = (<HTMLInputElement>(
      document.getElementById("diosEnSacramento")
    )).value;
    const diosEnVida = (<HTMLInputElement>document.getElementById("diosEnVida"))
      .value;
    const patronesComportamiento = (<HTMLInputElement>(
      document.getElementById("patronesComportamiento")
    )).value;
    const dialogoProfundo = (<HTMLInputElement>(
      document.getElementById("dialogoProfundo")
    )).value;
    const servidoresPostEncuentro = (<HTMLInputElement>(
      document.getElementById("servidoresPostEncuentro")
    )).value;
    const formacionAcompanantes = (<HTMLInputElement>(
      document.getElementById("formacionAcompanantes")
    )).value;
    const padreNuestro = (<HTMLInputElement>(
      document.getElementById("padreNuestro")
    )).value;
    const transmisionNacional = (<HTMLInputElement>(
      document.getElementById("transmisionNacional")
    )).value;
    const ciudadSeleccionada = (<HTMLInputElement>(
    ...
    if (this.httpOptions) {
      this.http
        .post(
          "https://encuentro-matrimonial-backend.herokuapp.com/formacion/matrimonio/create",
          jsonMatrimonio,
          this.httpOptions
        )
        .subscribe(
